...
export function Plan3DViewer({ plan }: { plan: PlanData }) {
  ...
  return (
    <div className="w-full h-[500px] rounded-lg overflow-hidden border border-border/40 bg-[#1a1a1a]">
      <Canvas camera={{ position: [plan.total_w, plan.total_w * 0.9, plan.total_h * 1.2], fov: 45 }} shadows>
        ...
        <group position={[-cx, 0, -cz]}>
          {/* Floor */}
          <mesh receiveShadow rotation={[-Math.PI / 2, 0, 0]} position={[plan.total_w / 2, 0, plan.total_h / 2]}>
            <planeGeometry args={[plan.total_w, plan.total_h]} />
            <meshStandardMaterial color="#d8cdb4" />
          </mesh>

          {plan.rooms.map((r) => (
            <RoomWalls key={r.id} room={r} plan={plan} />
          ))}
        </group>
...
function RoomWalls({ room: r, plan }: { room: PlanData["rooms"][number]; plan: PlanData }) {
  // For each of 4 walls, check openings on that wall to leave gaps.
  // Simple approach: split wall into segments around openings.
...
function WallSegment({ a, b, bottom, height }: { a: [number, number]; b: [number, number]; bottom: number; height: number }) {
  const dx = b[0] - a[0];
  const dz = b[1] - a[1];
  ...
  const mz = (a[1] + b[1]) / 2;
  return (
    <mesh position={[mx, bottom + height / 2, mz]} rotation={[0, -angle, 0]} castShadow receiveShadow>
